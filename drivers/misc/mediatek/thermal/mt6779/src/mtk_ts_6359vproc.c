...
static char g_bind0[20] = "sysrst.6359vproc";
...
(struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
		mtktspmic_dprintk("[mt6359vproc_bind] %s\n", cdev->type);
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		mtktspmic_info(
			"[mt6359vproc_bind] error binding cooling dev\n");
		return -EINVAL;
	}

	mtktspmic_dprintk("[mt6359vproc_bind] binding OK, %d\n", table_val);
	return 0;
}
			    ...
			    struct thermal_cooling_device *cdev)
{
	...
	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
		mtktspmic_dprintk("[mt6359vproc_unbind] %s\n", cdev->type);
	} else if (!strcmp(cdev->type, g_bind1)) {
		table_val = 1;
		mtktspmic_dprintk("[mt6359vproc_unbind] %s\n", cdev->type);
	} else if (!strcmp(cdev->type, g_bind2)) {
	...
	if (thermal_zone_unbind_cooling_device(thermal, table_val, cdev)) {
		mtktspmic_info(
			"[mt6359vproc_unbind] error unbinding cooling dev\n");
		return -EINVAL;
	}

	mtktspmic_dprintk("[mt6359vproc_unbind] unbinding OK\n");
	return 0;
}
...
{
	seq_printf(m,
		"[mt6359vproc_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,\n",
...
(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		down(&sem_mutex);
		mtktspmic_dprintk(
			"[mt6359vproc_write] mt6359vproc_unregister_thermal\n");
		...
		mtktspmic_dprintk(
			"[mt6359vproc_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,",
		...
		mtktspmic_dprintk(
			"[mt6359vproc_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,",
			g_bind0, g_bind1, g_bind2, g_bind3, g_bind4);
		...
		mtktspmic_dprintk(
			"[mt6359vproc_write] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,",
			trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3]);
		...
		mtktspmic_dprintk(
			"[mt6359vproc_write] mt6359vproc_register_thermal\n");
	...
	mtktspmic_dprintk("[mt6359vproc_write] bad argument\n");
    #ifdef CONFIG_MTK_AEE_FEATURE
...
static int mt6359vproc_register_thermal(void)
{
	mtktspmic_dprintk("[mt6359vproc_register_thermal]\n");
...
static void mt6359vproc_unregister_thermal(void)
{
	mtktspmic_dprintk("[mt6359vproc_unregister_thermal]\n");
...
static int __init mt6359vproc_init(void)
{
	...
	mtktspmic_info("[mt6359vproc_init]\n");

	err = mt6359vproc_register_cooler();
	if (err)
		return err;
	err = mt6359vproc_register_thermal();
	if (err)
		goto err_unreg;
...
{
	mtktspmic_info("[mt6359vproc_exit]\n");
	mt6359vproc_unregister_thermal();
	mt6359vproc_unregister_cooler();
