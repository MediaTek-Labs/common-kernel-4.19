...
static int __init amms_probe(struct platform_device *pdev)
{
	...
	version = (const char *)of_get_property(node, "version", NULL);
	if (!version) {
		pr_info("%s, lack of amms version info\n", __func__);
		return -EINVAL;
	}
	...
	if (strcmp("1", version) == 0) {
		...
		&pdev->dev, amms_irq_num, NULL, amms_legacy_handler,
		IRQF_ONESHOT|IRQF_TRIGGER_NONE, "amms_legacy_irq", NULL) != 0) {
			pr_info("Fail to request amms_legacy_irq interrupt!\n");
			return -EBUSY;
		}
	} else if (strcmp("2", version) == 0) {
		if (devm_request_threaded_irq(&pdev->dev,
		amms_irq_num, NULL, amms_handler,
		IRQF_ONESHOT|IRQF_TRIGGER_NONE, "amms_irq",
		NULL) != 0) {
			pr_info("Fail to request amms_irq interrupt!\n");
			return -EBUSY;
		}
	} else {
		pr_info("%s,amms version mismatch %s\n", __func__, version);
		return -EINVAL;
	}
