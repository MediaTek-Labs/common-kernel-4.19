...
extern struct dentry *midware_dbg_device;
static struct apusys_res_mgr g_res_mgr;

static const char dev_type_string[APUSYS_DEVICE_LAST][APUSYS_DEV_NAME_SIZE] = {
	"none",
	"sample",
	"mdla",
	"vpu",
...
static void secure_ws_lock(void)
{
#ifdef CONFIG_PM_WAKELOCKS
	mutex_lock(&ws_mutex);
...
static void secure_ws_unlock(void)
{
#ifdef CONFIG_PM_WAKELOCKS
	mutex_lock(&ws_mutex);
	...
	for (i = 0; i < APUSYS_DEVICE_MAX; i++) {
		...
		if (!list_empty(&tab->acq_list)) {
			...
			if (acq->is_done && acq->target_num == acq->acq_num
				...
				mdw_flw_debug("res tab(%d) has acq_list done\n",
					i);
...
int acq_device_async(struct apusys_dev_aquire *acq)
{
	...
	if (acq->acq_num < acq->target_num) {
		mdw_drv_debug("acquire device(%d/%d/%d) async...\n",
	...
	uint32_t boost_val, uint32_t timeout)
{
	...
	mdw_flw_debug("poweron dev(%d-#%u) boost(%u) timeout(%u)\n",
...
int res_power_off(int dev_type, uint32_t idx)
{
	...
	mdw_flw_debug("powerdown dev(%d-#%u)\n", dev_type, idx);
	...
	while (1) {
		...
		for (i = 0; i < tab->dev_num; i++) {
			...
			if (ret) {
				...
				mdw_flw_debug("suspend dev(%s-#%d) done\n",
					tab->name, i);
			}
			mutex_unlock(&g_res_mgr.mtx);
		}
	...
	while (1) {
		...
		for (i = 0; i < tab->dev_num; i++) {
			...
			if (ret) {
				...
				mdw_flw_debug("resume dev(%s-#%d) done\n",
					tab->dev_list[i].name, i);
			}
		}
		dev_type++;
...
int res_secure_on(int dev_type)
{
	...
	case APUSYS_DEVICE_SAMPLE:
		ret = 0;
		mdw_drv_debug("[sec]dev(%d) secure mode on(%d)\n",
			dev_type, ret);
		break;
	...
	case APUSYS_DEVICE_VPU:
		...
		mdw_drv_warn("[sec]dev(%d) secure mode on(%d)\n",
			dev_type, ret);
		if (!ret)
			secure_ws_lock();
		break;
...
int apusys_register_device(struct apusys_device *dev)
{
	...
	if (tab != NULL) {
		...
		mdw_flw_debug("add dev to type(%d) list\n", dev->dev_type);
		for (i = 0; i < APUSYS_DEV_TABLE_MAX; i++) {
			...
			mdw_flw_debug("register dev(%d-#%d/%d/%p) ok\n",
				dev->dev_type, i,
				tab->dev_num, dev);
		...
		mdw_flw_debug("register device(%d)\n", dev->dev_type);
	...
	for (i = 0; i <= dev->preempt_level; i++) {
		ret = thread_pool_add_once();
		if (ret) {
			mdw_drv_err("create thread(%d) to pool fail(%d)\n",
				i, ret);
...
int apusys_unregister_device(struct apusys_device *dev)
{
	...
	for (i = 0; i < APUSYS_DEV_TABLE_MAX; i++) {
		if (tab->dev_list[i].dev == dev) {
			mdw_flw_debug("get dev(%p)\n", dev);
			if (tab->dev_list[i].cur_owner ==
				APUSYS_DEV_OWNER_NONE) {
				mdw_flw_debug("dev(%p) is free, can release\n",
					tab->dev_list[i].dev);
