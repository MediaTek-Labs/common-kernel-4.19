...
static void deadline_load_tracing(struct work_struct *data)
{
	...
	mdw_flw_debug("%s: %llu, %llu, %llu, boost: %llu\n",
		__func__,
		root->avg_load[0],
		root->avg_load[1],
		root->avg_load[2],
...
int deadline_queue_init(int type)
{
	...
	if (first_boot) {
		apusys_dbg_deadline =
			debugfs_create_dir("deadline", midware_dbg_root);
...
int deadline_task_start(struct apusys_subcmd *sc)
{
	...
	if (tab->dev_num <= load && root->load_boost == 0) {
		root->load_boost = 1;
		mdw_flw_debug("load_boost: %llu, %llu, %llu, %llu\n",
			sc->runtime, sc->period,
			root->total_period, root->total_runtime);
	}
