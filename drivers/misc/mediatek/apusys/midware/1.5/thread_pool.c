...
void thread_pool_dump(void)
{
	...
	mdw_flw_debug("=====================================\n");
	mdw_flw_debug("| apusys thread pool status: total(%d)\n",
		g_pool_mgr.total);
	list_for_each_safe(list_ptr, tmp, &g_pool_mgr.thread_list) {
		...
		mdw_flw_debug(" thread idx = %d\n", i);
		mdw_flw_debug(" status     = %d\n", inst->status);
		if (sc == NULL || inst->status == APUSYS_THREAD_STATUS_IDLE)
			continue;
		mdw_flw_debug(" cmd        = 0x%llx\n", cmd->cmd_id);
		mdw_flw_debug(" subcmd     = %d/%p\n", sc->idx, sc);
		mdw_flw_debug(" stop       = %d\n", inst->stop);
	}
	mdw_flw_debug("=====================================\n");
