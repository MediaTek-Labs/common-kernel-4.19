...
#include "mnoc_api.h"
#endif
#ifdef APUSYS_OPTIONS_INF_REVISER
...
{
#ifndef APUSYS_OPTIONS_INF_REVISER
	unsigned long ret_ctx = 0;

	mutex_lock(&g_ctx_mgr.mtx);
	ret_ctx = find_first_zero_bit(g_ctx_mgr.ctx, 32);
	if (ret_ctx >= 32)
		ret_ctx = -1;
	else
		bitmap_set(g_ctx_mgr.ctx, ret_ctx, 1);
	...
	if (req_size == 0) {
		...
		mdw_drv_debug("tcm default request size(0x%x)\n", req_size);
	}
...
static int insert_pack_cmd(struct apusys_subcmd *sc, struct pack_cmd **ipc)
{
	...
	if (bit0 && bit1) {
		mdw_drv_debug("pack cmd satified(0x%llx/0x%llx)(%d/%d)\n",
			sc->par_cmd->hdr->uid,
			sc->par_cmd->cmd_id,
			sc->idx,
			pack_idx);
		...
		if (pc == NULL) {
			mdw_drv_err("alloc packcmd(0x%llx/%d) for execute fail\n",
...
static int clear_pack_cmd(struct apusys_cmd *cmd)
{
	...
	mdw_drv_debug("0x%llx cmd clear pack list\n",
		cmd->cmd_id);
...
static int exec_cmd_func(void *isc, void *idev_info)
{
	...
	/* call execute */
#define SCHED_TRACE_STR "apusys_scheduler|dev: %d_%d,"\
	" pid: %d_%d, cmd_id: 0x%08llx"
	midware_trace_begin(SCHED_TRACE_STR,
...
#define APUSYS_PRINT_EXECINFO "pid(%d/%d) 0x%llx/0x%llx-#%d(%u)"\
	...
	mdw_drv_debug(APUSYS_PRINT_EXECINFO,
		sc->par_cmd->pid,
		sc->par_cmd->tgid,
		sc->par_cmd->hdr->uid,
		sc->par_cmd->cmd_id,
...
#define APUSYS_PRINT_EXECFAIL "pid(%d/%d) 0x%llx/0x%llx-#%d(%u)"\
	...
	" ctx(%u/%d/0x%x/0x%x) time(%u/%u) fail(%d)\n"
		mdw_drv_err(APUSYS_PRINT_EXECFAIL,
			sc->par_cmd->pid,
			sc->par_cmd->tgid,
			sc->par_cmd->hdr->uid,
			sc->par_cmd->cmd_id,
...
#define APUSYS_PRINT_EXECOK "pid(%d/%d) 0x%llx/0x%llx-#%d(%u)"\
	...
	" ctx(%u/%d/0x%x/0x%x) time(%u/%u) done\n"
		mdw_drv_debug(APUSYS_PRINT_EXECOK,
			sc->par_cmd->pid,
			sc->par_cmd->tgid,
			sc->par_cmd->hdr->uid,
			sc->par_cmd->cmd_id,
...
out:
	...
	midware_trace_end(SCHED_TRACE_STR,
			dev_info->dev->dev_type,
			dev_info->dev->idx,
			sc->par_cmd->pid,
			sc->par_cmd->tgid,
			sc->par_cmd->cmd_id);
#undef SCHED_TRACE_STR
		...
		while (!bitmap_empty(available, APUSYS_DEV_TABLE_MAX)
			...
			if (get_pack_idx(sc) != VALUE_SUBGRAPH_PACK_ID_NONE) {
				...
				if (ret < 0) {
					...
					if (pc->acq.acq_num ==
						pc->acq.target_num) {
						mdw_drv_debug("execute pack cmd\n");
			...
			if (ret < 0 || acq.acq_num <= 0) {
				mdw_drv_debug("no dev(%d) available\n", type);
				mutex_lock(&sc->mtx);
...
int apusys_sched_del_cmd(struct apusys_cmd *cmd)
{
	...
	mdw_flw_debug("wait 0x%llx cmd done...\n",
		cmd->cmd_id);
...
int apusys_sched_add_cmd(struct apusys_cmd *cmd)
{
	...
	while (i < cmd->hdr->num_sc) {
		...
		ret = apusys_subcmd_create(i, cmd, &sc, dp_ofs);
		if (ret) {
			mdw_drv_err("create sc for cmd(%d/%p) fail(%d)\n",
				i, cmd, ret);
			break;
		}
