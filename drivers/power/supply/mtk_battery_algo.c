	...
	int *is_ascending, int *is_descending)
{
	...
	*is_descending = 0;
	...
	pr_debug("act:%d table: %d %d %d %d %d %d %d %d %d %d\n",
	...
	pr_debug("active_table_no is %d, %d %d\n",
		ptable->active_table_number,
		*is_ascending,
		*is_descending);
	for (i = 0; i < ptable->active_table_number; i++) {
...
void fgr_construct_battery_profile(struct mtk_battery *gm, int table_idx)
{
	...
	if (temp_profile_p == NULL) {
		pr_debug("[FGADC] fg_get_profile : create table fail !\n");
		return;
	}

	if (fg_check_temperature_order(gm, &is_ascending, &is_descending)) {
		pr_notice("[FGADC] fg_check_temperature_order : t0~t3 setting error !\n");
		return;
	}
	...
	if (table_idx == ptable->temperature_tb0) {
		...
		pr_debug("[Profile_Table]pseudo1_en:[%d] lowT %d %d %d lowPs1 %d highPs1 %d batterypseudo1_h [%d]\n",
			pdata->pseudo1_en, low_temp,
			high_temp, temperature,
			low_pseudo1, high_pseudo1,
			algo->batterypseudo1_h);
		pr_debug("[Profile_Table]pseudo100_en:[%d] %d lowT %d %d %d low100 %d %d [%d]\n",
 ...
 */
		...
		pr_debug("[Profile_Table]lowT %d %d %d lowQ %d %d qmax_t_0ma_h [%d]\n",
			low_temp, high_temp, temperature,
			UNIT_TRANS_10 * low_qmax,
			UNIT_TRANS_10 * high_qmax,
			algo->qmax_t_0ma_h);
		pr_debug("[Profile_Table]lowT %d %d %d lowQh %d %d qmax_t_Nma_h [%d]\n",
			low_temp, high_temp, temperature,
		...
		pr_debug("[Profile_Table]lowT %d %d %d LowShutZCV %d HighShutZCV %d shutdown_hl_zcv [%d]\n",
			low_temp, high_temp, temperature,
 ...
 */
	...
	pr_debug("[Profile_Table]T_table %d T_table_c %d %d %d is_ascend %d %d\n",
		algo->T_table, algo->T_table_c, pdata->pseudo1_en,
		pdata->pseudo100_en, is_ascending, is_descending);
	pr_debug("[Profile_Table]Pseudo1_h %d %d, Qmax_T_0mA_H %d,%d qmax_t_0ma_h_tb1 %d %d\n",
		algo->batterypseudo1_h,
		algo->batterypseudo100,
		algo->qmax_t_0ma_h,
		algo->qmax_t_Nma_h,
	...
	struct mtk_battery *gm, bool update, int table_idx)
{
	...
	if (fg_temp != algo->last_temp || update == true) {
		pr_notice("[%s] tempture from(%d)to(%d) Tb:%d",
			__func__,
	...
	int qmax, int table_index)
{
	...
	pr_debug("[%s] qmax:%d qmax_t_0ma:%d\n",
		__func__,
	...
	int _vboot, int table_index)
{
	...
	if (table_index == ptable->temperature_tb0) {
		...
		if (algo->qmax_t_0ma < 3000) {
			pr_notice("[ERR][%s]index %d idx:%d _vboot:%d %d qmax_t_0ma:[%d => 3000]\n",
				__func__,
				table_index, j,
		...
		if (algo->qmax_t_0ma > 50000) {
			pr_notice("[ERR][%s]index %d idx:%d _vboot:%d %d qmax_t_0ma:[%d => 50000]\n",
				__func__,
				table_index, j,
				_vboot, profile_p[j].voltage, algo->qmax_t_0ma);
		}
		...
		if (algo->qmax_t_0ma_tb1 < 3000) {
			pr_notice("[ERR][%s]index %d idx:%d _vboot:%d %d qmax_t_0ma_tb1:[%d => 3000]\n",
				__func__,
				table_index, j,
		...
		if (algo->qmax_t_0ma_tb1 > 50000) {
			pr_notice("[ERR][%s]index %d idx:%d _vboot:%d %d qmax_t_0ma_tb1:[%d => 50000]\n",
				__func__,
				table_index, j,
				_vboot, profile_p[j].voltage,
				algo->qmax_t_0ma_tb1);
	...
	if (table_index == ptable->temperature_tb1) {
		pr_debug("[%s]index %d idx:%d _vboot:%d %d qmax_t_0ma_tb1:%d\n",
			__func__,
			table_index, j, _vboot,
			profile_p[j].voltage, algo->qmax_t_0ma_tb1);
	} else {
		pr_debug("[%s]index %d idx:%d _vboot:%d %d qmax_t_0ma:%d\n",
			__func__,
			table_index, j, _vboot,
	...
	int oriv, int curr, int tablei)
{
	...
	if (profile_p == NULL) {
		pr_notice("[ERR][%s] fail ,profile_p=null!\n",
			__func__);
		return 0;
	}
	size = fg_get_saddles(gm);

	pr_debug("[%s]size:%d oriv=%d I:%d\n",
		__func__,
		size, oriv, curr);
	...
	for (; high > 0; high--) {
		if (high >= 1) {
			...
			if (fg_volt_withIR > oriv) {
				...
				pr_notice("[%s]h_percent=[%d,%d],high=%d,fg_volt_withIR=%d > oriv=%d\n",
					__func__,
			...
			pr_notice("[ERR][%s] can't find available voltage!!!\n",
			__func__);
	...
	for (i = hit_h_percent; i >= hit_l_percent; i = i-10) {
		...
		if (fg_volt_withIR > oriv) {
			pr_notice("[%s]fg_volt=%d,%d,IR=%d,orig_v:%d,+IR=%d,percent=%d,\n",
				__func__,
				fg_volt, high,
	...
	pr_notice("[ERR][%s] should not reach here!!!!!!\n",
		__func__);
	return fg_volt;
}
...
void fgr_construct_vboot(struct mtk_battery *gm, int table_idx)
{
	...
	pr_debug("[%s] idx %d T_NEW %d T_table %d T_table_c %d qmax_sel %d\n",
		__func__,
		table_idx, curr_temp,
	...
	if (pdata->qmax_sel == 0) {
		...
		pr_debug("[%s]idx %d T_NEW %d T_table %d T_table_c %d qmax_sel %d vboot_t=[%d:%d:%d] %d %d rac %d\n",
			__func__,
			table_idx, curr_temp,
			algo->T_table, algo->T_table_c,
...
/* batterypseudo1_auto = get_batterypseudo1_auto(vboot, shutdown_hl_zcv); */
	...
	pr_debug("[%s] qmax_sel=%d iboot_sel=%d iboot:%d vbat:%d i:%d vboot:%d %d %d\n",
		__func__,
		pdata->qmax_sel, pdata->iboot_sel, iboot,
		ptim_vbat, ptim_i, algo->vboot, algo->vboot_c, vboot_t);

	if (pdata->qmax_sel == 0) {
		pr_debug("[%s][by_qmax]qmax_sel %d qmax %d vboot %d %d pmic_min_vol %d iboot %d r %d\n",
			__func__,
	...
	if (pdata->qmax_sel == 1) {
		pr_debug("[%s][by_vboot]qmax_sel %d vboot_t %d vboot %d %d pmic_min_vol %d iboot %d rac %d\n",
			__func__,
...
void fgr_dump_table(struct mtk_battery *gm, int idx)
{
	...
	pr_notice("[%s]table idx:%d (i,mah,voltage,resistance,percentage)\n",
		__func__,
		idx);
	for (i = 0; i < fg_get_saddles(gm); i = i + 5) {
		pr_notice("(%2d,%5d,%5d,%5d,%3d)(%2d,%5d,%5d,%5d,%3d)(%2d,%5d,%5d,%5d,%3d)(%2d,%5d,%5d,%5d,%3d)(%2d,%5d,%5d,%5d,%3d)\n",
...
void fgr_update_quse(struct mtk_battery *gm, int caller)
{
	...
	if (caller == 1) {
		pr_debug("[%s]aging_sel %d qmax_t_0ma_tb1 %d quse_tb1 [%d] aging[%d]\n",
			__func__,
...
void fgr_update_uisoc_threshold(struct mtk_battery *gm)
{
	...
	pr_debug("[%s]car:%d quse_tb1[%d %d] gap[%d %d][%d]\n",
		__func__,
...
void fgr_update_uisoc_ht(struct mtk_battery *gm)
{
	...
	pr_debug("[%s] update ht_en:%d ht_gap:%d\n",
		__func__,
...
void fgr_update_uisoc_lt(struct mtk_battery *gm)
{
	...
	pr_debug("[%s] update lt_en:%d lt_gap:%d\n",
		__func__,
...
int SOC_to_OCV_c(struct mtk_battery *gm, int _soc)
{
	...
	if (profile_p == NULL) {
		pr_notice("[%s] fgauge get c table: fail !\n",
			__func__);
		return 0;
	}
	...
	pr_debug("[%s]soc:%d dod:%d! voltage:%d highidx:%d\n",
		__func__,
...
int DOD_to_OCV_c(struct mtk_battery *gm, int _dod)
{
	...
	pr_debug("[%s]DOD_to_OCV: dod:%d vol:%d highidx:%d\n",
		__func__,
		_dod, ret_vol, high);
...
int OCV_to_SOC_c(struct mtk_battery *gm, int _ocv)
{
	...
	profile_p = fg_get_profile(gm, ptable->temperature_tb1);
	if (profile_p == NULL) {
		pr_notice("[%s]can't get c table: fail\n",
			__func__);
		return 0;
	}
	...
	pr_debug("[%s]voltage:%d dod:%d highidx:%d\n",
		__func__,
		_ocv, ret_vol, high);
...
int OCV_to_DOD_c(struct mtk_battery *gm, int _ocv)
{
	...
	profile_p = fg_get_profile(gm, ptable->temperature_tb1);
	if (profile_p == NULL) {
		pr_notice("[%s] fgauge can't get c table: fail\n",
			__func__);
		return 0;
	}
...
void fgr_update_c_dod(struct mtk_battery *gm)
{
	...
	pr_debug("[%s] fg_c_dod %d fg_c_d0_dod %d car %d quse_tb1 %d fg_c_soc %d\n",
		__func__,
...
void fgr_dod_init(struct mtk_battery *gm)
{
	...
	if (algo->rtc_ui_soc == 0 || con0_soc == 0) {
		...
		if (algo->rtc_ui_soc < 0) {
			pr_notice("[%s]rtcui<0,set to 0,rtc_ui_soc:%d fg_c_d0_soc:%d\n",
				__func__,
		...
		pr_notice("[%s]rtcui=0 case,init_swocv=%d,OCV_to_SOC_c=%d ui:[%d %d] con0_soc=[%d %d]\n",
			__func__,
	...
	pr_notice("[%s]fg_c_d0[%d %d %d] d0_sel[%d] c_soc[%d %d] ui[%d %d] soc[%d] con0[ui %d %d]\n",
		__func__,
...
void fgr_int_end_flow(struct mtk_battery *gm, unsigned int intr_no)
{
	...
	default:
		...
		pr_notice("[Intr_Number_to_Name] unknown intr %d\n",
			intr_no);
		break;
	}
	...
	pr_debug("[%s][%s]soc:%d, c_soc:%d ui_soc:%d VBAT %d T:[%d C:%d] car:%d\n",
		__func__,
...
void fgr_update_fg_bat_int1_threshold(struct mtk_battery *gm)
{
	...
	pr_debug("[%s] quse_tb1:%d gap:%d diff_soc_setting:%d MIN:%d\n",
		__func__,
...
void fgr_bat_int2_h_handler(struct mtk_battery *gm)
{
	...
	pr_debug("[%s][IN]ui_soc %d, ht_gap:[%d %d], _car[%d %d %d]\n",
		__func__,
	...
	pr_debug("[%s][OUT]ui_soc %d, ui_gap_ht:%d, _car[%d %d %d]\n",
...
void fgr_bat_int2_l_handler(struct mtk_battery *gm)
{
	...
	pr_debug("[%s][IN]ui_soc %d, lt_gap[%d %d] _car[%d %d %d]\n",
		__func__,
	...
	if (ui_gap_lt < 0) {
		pr_debug("[FG_ERR][%s] ui_gap_lt %d should not less than 0\n",
			__func__,
			ui_gap_lt);
		ui_gap_lt = 0;
	}
	...
	pr_debug("[%s][OUT]ui_soc %d, ui_gap_lt:%d, _car[%d %d %d]\n",
...
void fgr_bat_int2_handler(struct mtk_battery *gm, int source)
{
	...
	pr_debug("[%s]car:%d pre_car:%d ht:%d lt:%d u_type:%d source:%d\n",
...
void fgr_time_handler(struct mtk_battery *gm)
{
	...
	pr_debug("[%s][IN] chr:%d, low_tracking:%d ui_soc:%d\n",
...
void fgr_vbat2_l_int_handler(struct mtk_battery *gm)
{
	...
	pr_debug("[%s]fg_vbat2_lt=%d %d,[%d %d %d]\n",
		__func__,
		algo->fg_vbat2_lt, algo->fg_vbat2_ht,
...
void fgr_set_int1(struct mtk_battery *gm)
{
	...
	battery_set_property(BAT_PROP_COULOMB_INT_GAP,
		algo->fg_bat_int1_gap);
	pr_debug("[%s]set cgap :fg_bat_int1_gap %d to kernel done\n",
		__func__,
		algo->fg_bat_int1_gap);
...
void battery_algo_init(struct mtk_battery *gm)
{
	...
	pr_notice("MTK Battery algo init bat_exist:%d\n",
		is_bat_exist);
	...
	pr_notice("[battery_recovery] is_evb:%d is_bat_exist %d\n",
